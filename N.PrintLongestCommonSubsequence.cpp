...
string printLCS(string a, string b, int n, int m, int dp[][100]){
	...
	string ans="";
	while(i!=0 || j!=0){
		if(a[i-1]==b[j-1]){                    // If both string's letter is same , then add in ans
			ans = a[i-1] + ans;
			i--;
			j--;
		}else{
			if(dp[i-1][j]>dp[i][j-1]){        //Else traverse into table
				i--;
			}else{
				j--;
			}
...
//	int dp[n+1][m+1]={-1};
	...
	int ans = LCS(a,b,n,m,dp);      // Build table from LCS fn
	...
	string res = printLCS(a,b,n,m,dp);    //Find string from this fn
	cout<<res;
	return 0;
}
